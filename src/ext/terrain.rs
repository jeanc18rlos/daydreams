...
//!
//! [`height`] is the single source of truth. The tile mesh is generated from it (`daydreams gen-terrain`),
...
/// Generate `Meshes/meadow_tile.obj` from [`height`]. Run with `daydreams gen-terrain`.
///
...
pub fn generate() -> std::io::Result<()> {
    ...
    let path = crate::app::assets::path(&format!("Meshes/{TILE_MESH}"));
    std::fs::write(&path, s)?;
    let ny = 1.0 / (1.0 + worst * worst).sqrt();
    log::info!(
        "{}: {} verts, {} tris, {} colliders; steepest normal.y {ny:.2}",
        path.display(),
        (VIS_N + 1) * (VIS_N + 1),
        2 * VIS_N * VIS_N,
        cverts.len() / 3
...
mod tests {
    ...
    fn tile_matches_height_field() {
        let path = crate::app::assets::path(&format!("Meshes/{TILE_MESH}"));
        let src = std::fs::read_to_string(&path).expect("run `daydreams gen-terrain`");
        let step = PERIOD / VIS_N as f32;
        let mut checked = 0;
