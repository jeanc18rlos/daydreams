...
at 1.0 ms and the meadow spawn (the door drawing the hall) within noise of its 2.2 ms. The
last one on the north wall wears [the key](#the-key-in-the-painting--extkeyrs-extpaintingrs-shaderspaintingfrag).
...
Six flags are hidden from `--help` because they are tools rather than features: `--panic-test`
...
[Elevator](#elevator)), and `--hold-key` and `--e-at FRAME` (with `--scene`: start with the
painting's key in hand, and press E on that frame as the keyboard would -- the press slot,
seen by the frame's first fixed step and latched for the grab; see
