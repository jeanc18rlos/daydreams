...
| `object.rs` | `UpdateCtx` carries the player's eye transform, so room logic can see where you look; `ObjectT::trimesh()` for triangle-mesh scenery |
| `engine.rs` | one `ext` field, table-driven scene keys, a grab tick, scene-load notification, and the triangle-mesh rounds in the collision pass |
| `shader.rs` | by-name uniform setters, with their locations memoised per program |
