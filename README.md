...
`cargo test --release` — 372 passed, 0 failed.
...
snapshot of what the objects answer at load, so collision that comes and goes or moves keeps
out of it by answering `ObjectT::static_collision` false: the elevator's leaves, which block
only while the doors are shut -- a ride arrives with them shut, and a snapshot that took them
would keep an invisible wall across the doorway after they opened -- and the window, which is
carried about. A prop that leaves the building through a gap in the scan falls past the carpet
...
| `input.rs` | four analog fields, filling the `//Joystick //TODO:` slot; `E`/`M`/`R`/`F`/`G` and the scene keys `8`–`.` (`8` `9` `0` `-` `=` `[` `]` `\` `;` `'` `,` `.`); `Shift` into the `VK_SHIFT` slot and the resolved sprint multipliers; a `wheel` accumulator in notches, cleared with the other edges by `EndFrame` |
...
Eleven flags are hidden from `--help` because they are tools rather than features: `--panic-test`
...
and `--stow-at FRAMES` / `--drop-at FRAMES` (with `--scene`: press F and G on each frame they
name -- `--stow-at 60,120` -- the same way, so a stow, the retrieve after it and a put-down can
be driven headlessly; see [Inventory](#inventory--extinventoryrs)).
