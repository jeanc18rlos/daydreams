...
[Elevator](#elevator)), and `--window-scale S` and `--unlock-window` (with `--scene`: how the
Backrooms' window is built; see [The window](#the-window)). `--view-glb` excludes `--scene`;
its path is taken under the working directory when a file is there, under the asset root
otherwise.
