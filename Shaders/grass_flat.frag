...
void main(void) {
	...
	// Triplanar pick: this material also dresses the tunnel embankments, whose walls are
	// vertical. Map each face from the world plane its normal faces, so walls get an upright
	// grass pattern instead of the ground's XZ pattern smeared into vertical streaks. Flat
	// per-face normals make this a clean per-face choice with no blending seams.
	vec3 an = abs(n);
	vec2 uv = (an.y >= an.x && an.y >= an.z) ? ex_world.xz
	        : (an.x >= an.z)                  ? ex_world.zy
	        :                                   ex_world.xy;
	...
	float ao = 1.0; // flat ground: no valleys to occlude
